...
void validar_entrada(int argc, char** argv){
		...
		asprintf(&toBison, "%s%s",toBison,nombre_archivo);
...
void realizar_lectura(FILE* archivo_actual){	
	...
	while(fgets(buff, 255, (FILE*)archivo_actual)){
		//printf("%s\n",buff);
...
void validar_include(char* tira){
	...
	n++;
	    ...
	    n--;
		if(tira[n+1] == 'i' && tira[n+2] == 'n' && tira[n+3] == 'c' && tira[n+4] == 'l' && tira[n+5] == 'u' && tira[n+6] == 'd' && tira[n+7] == 'e' && tira[n+8] == ' '){
			n=n+9;}
	}
	 int indice =0;
